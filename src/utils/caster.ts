...
export function bytesToVec(str: `0x${string}`): number[] {
  const sliced: string = str.slice(2);
  if (sliced.length % 2 != 0) throw new Error("not bytes");

  const result: number[] = [];
  for (let i = 0; i < sliced.length; i += 2) {
    const byte = parseInt('0x' + sliced.slice(i, i + 2));
    result.push(byte);
  }
  return result;
}
